...
<body>
  <h1>Salamanders</h1> <?php foreach ($salamanders as $s): ?> <h2><?= htmlspecialchars($s['name']) ?></h2>
    <p> </p>
    <p> </p> <strong>Habitat:</strong> <?= nl2br(htmlspecialchars($s['habitat'])) ?> <?= nl2br(htmlspecialchars($s['description'])) ?>
    <hr> 
    <?php endforeach; ?>
</body>
